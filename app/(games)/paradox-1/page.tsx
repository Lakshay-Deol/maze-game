...
export default function Paradox1Page() {
  const [input, setInput] = useState("");
  const router = useRouter();
  const [count, setCount] = useState(() => {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("paradox1_attempts");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (["github"].includes(answer)) {
      localStorage.setItem("score", JSON.stringify(score + 1));
      localStorage.removeItem("paradox1_attempts");
      router.push("/");
    } else {
      alert("Incorrect Answer. Please Try Again.");
      attempts = count - 1;
      setCount(attempts);
      localStorage.setItem("paradox1_attempts", attempts.toString());
      if (attempts === 0) {
        localStorage.setItem("score", JSON.stringify(score - 1));
        localStorage.removeItem("paradox1_attempts");
        router.push("/");
