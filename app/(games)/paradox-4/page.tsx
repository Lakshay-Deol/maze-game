...
export default function Riddle1Page() {
  ...
  const [count, setCount] = useState(() => {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("paradox4_attempts");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (["samayraina"].includes(answer)) {
      localStorage.setItem("score", JSON.stringify(score + 1));
      localStorage.removeItem("paradox4_attempts");
      router.push("/");
    } else {
      alert("Incorrect Answer. Please Try Again.");
      attempts = count - 1;
      setCount(attempts);
      localStorage.setItem("paradox4_attempts", attempts.toString());
      if (attempts === 0) {
        localStorage.setItem("score", JSON.stringify(score - 1));
        localStorage.removeItem("paradox4_attempts");
        router.push("/");
